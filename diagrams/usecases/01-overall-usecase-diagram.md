...
## Mô Tả
Biểu đồ này thể hiện tổng quan toàn bộ hệ thống với tất cả các tác nhân chính và 22 use case khái quát nhất.
...
- **UC-20: Quản lý dữ liệu mạng lưới đường sắt** (Railway Network Data Management): Quản lý dữ liệu mạng lưới gồm Ga dừng (`Station`), Tuyến đường (`Route`), phiên bản network và mạng lưới ray GeoJSON (`RailwayLine`). Tương ứng với các module `/station`, `/route` và `/railway-network` của backend.
