...
graph TB
    ...
    AutoStatus -.cập nhật tự động.-> TripStatus[Trạng thái chuyến: SCHEDULED → IN_PROGRESS → COMPLETED]
    ...
    class AutoStatus,TripStatus autoSystem
...
- **Nghiệp vụ chi tiết:** Lái tàu chọn cụ thể vị trí ghế bị hỏng trên sơ đồ trực quan của toa, mô tả tình trạng lỗi và tải ảnh chụp hiện trạng sự cố lên hệ thống. Báo cáo này sẽ được chuyển trạng thái `PENDING` và kích hoạt thông báo (qua **Notification Service**) tới Quản trị viên để Admin thực thi quy trình xử lý sự cố ghế hỏng cho khách hàng (`UC-21`).

---

## Cơ Chế Tự Động Cập Nhật Trạng Thái Chuyến Tàu
Để tối giản thao tác thủ công của Lái tàu, hệ thống **không yêu cầu** lái tàu phải bấm nút bắt đầu hay kết thúc hành trình. Trạng thái chuyến tàu được luân chuyển tự động theo thời gian vận hành:

```mermaid
graph LR
    SCHEDULED[SCHEDULED<br/>Đã lên lịch] -->|Đến giờ khởi hành| IN_PROGRESS[IN_PROGRESS<br/>Đang chạy]
    IN_PROGRESS -->|Đến giờ kết thúc| COMPLETED[COMPLETED<br/>Hoàn thành]
...
```
- **SCHEDULED $\rightarrow$ IN_PROGRESS:** Khi thời gian thực tế chạm vào giờ khởi hành, trạng thái sẽ đổi sang đang chạy.
