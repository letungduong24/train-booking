...
function PassengersPageContent() {
    ...
    const fromStation = metadata?.resolvedFromStation ?? (trip.route.stations || []).find((rs: any) => rs.stationId === fromStationId);
    const toStation = metadata?.resolvedToStation ?? (trip.route.stations || []).find((rs: any) => rs.stationId === toStationId);
    ...
    return (
        <div className="flex flex-1 flex-col gap-4 py-4 md:gap-8">
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl px-6 py-5 shadow-sm border border-gray-100 dark:border-zinc-800 relative overflow-hidden mb-8">
                <div className="flex flex-row items-start justify-between gap-4 relative z-10 mb-5">
                    <div className="space-y-1">
                        <h2 className="text-lg font-semibold text-[#802222] dark:text-rose-400 leading-6">Tóm tắt chuyến đi</h2>
                        <p className="text-xs font-medium text-muted-foreground">Thông tin đặt vé của bạn</p>
                    </div>
                    <Badge variant="outline" className="border-[#802222]/20 text-[#802222] dark:text-rose-400 font-medium px-3 py-1 rounded-full text-xs">
                        Tàu {trip.train.code}
                    </Badge>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5 relative z-10">
                    <div className="space-y-1">
                        <p className="text-xs font-medium text-muted-foreground">Ga đi</p>
                        <p className="font-semibold text-sm text-gray-950 dark:text-white">{fromStation?.station?.name || 'Đang cập nhật'}</p>
                    </div>
                    <div className="space-y-1">
                        <p className="text-xs font-medium text-muted-foreground">Ga đến</p>
                        <p className="font-semibold text-sm text-gray-950 dark:text-white">{toStation?.station?.name || 'Đang cập nhật'}</p>
                    </div>
                    <div className="space-y-1">
                        <p className="text-xs font-medium text-muted-foreground">Khởi hành</p>
                        <div className="flex items-center gap-2">
                            <Calendar className="h-3.5 w-3.5 text-[#802222]/40" />
                            <span className="font-semibold text-sm text-gray-950 dark:text-white tabular-nums">{format(departureDate, 'HH:mm dd/MM')}</span>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <p className="text-xs font-medium text-muted-foreground">Dự kiến đến</p>
                        <div className="flex items-center gap-2">
                            <Clock className="h-3.5 w-3.5 text-[#802222]/40" />
                            <span className="font-semibold text-sm text-gray-950 dark:text-white tabular-nums">
                                {arrivalDate ? format(arrivalDate, 'HH:mm dd/MM') : 'Đang cập nhật'}
                            </span>
                        </div>
                    </div>
                </div>
