...
export class TripCron {
  ...
  constructor(
    ...
    const intervalMinutes = this.configService.get<number>(
      'TRIP_STATUS_UPDATE_INTERVAL_MINUTES',
      1,
    );
    this.logger.log(
      `Trip status auto-update will run every ${intervalMinutes} minute(s)`,
    );
  }
  ...
  async autoUpdateTripStatus() {
    ...
    try {
      ...
      if (startedCount > 0) {
        this.logger.log(
          `Started ${startedCount} trips (SCHEDULED → IN_PROGRESS)`,
        );
      }
      ...
      if (completedCount > 0) {
        this.logger.log(
          `Completed ${completedCount} trips (IN_PROGRESS → COMPLETED)`,
        );
      }
